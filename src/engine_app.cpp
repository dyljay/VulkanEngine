...
void EngineApp::run()
{
    ...
    // skybox texture
    auto cubeMap =
        EngineTexture::createCubeMap(geDevice, cubeTextureFilePaths);
    ...
    for (int i = 0; i < descriptorSets.uboSets.size(); i++) {
        ...
        EngineDescriptorWriter(*descriptorSetLayouts.uboSetLayout,
                               *globalPool)
    ...
    while (!shouldQuit) {
        ...
        while (SDL_PollEvent(&e) != 0) {
            ...
            if (e.type == SDL_EVENT_MOUSE_MOTION && !userSeeMouse) {
                cameraController.handleMouseMovements(e,
                                                      frameTime,
                                                      viewerObject);
            }
        ...
        if (auto commandBuffer = geRenderer.beginFrame()) {
            ...
            if (hasClicked) {
                        ...
                        offscreenRenderer.beginFrame())
                {
                    ...
                    id_t objIDFromPixel =
                        *offscreenRenderer.getPixelData<uint32_t>(x, y);
...
void EngineApp::populateMatTexDescriptorSets(
    ...
    DescriptorSetLayouts& descriptorSetLayouts)
{
    descriptorSets.materialSets.resize(
        PBRMaterial::return_total_material());
    ...
    for (auto& kv : geObjects) {
        ...
        for (std::shared_ptr<PBRMaterial>& material : obj.model->materials)
        {
            ...
            auto bufferInfo =
                material->getMaterialBuffer()->descriptorInfo();
...

    {
        ...
        EngineDescriptorWriter(*descriptorSetLayouts.textureLayout,
                               *globalPool)
...
void EngineApp::loadGameObjects()
{
    ...
    girl.transform.rotation = {glm::radians(-90.f),
                               glm::radians(180.f),
                               0.0f};
