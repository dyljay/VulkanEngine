...
{
    return VkDescriptorImageInfo{textureSampler,
                                 textureImageView,
...
void EngineTexture::upload2D(int w, int h, stbi_uc* pixels)
{
    ...
    geDevice.copyBufferToImage(stagingBuffer.getBuffer(),
                               textureImage,
                               texWidth,
                               texHeight,
                               1);
...
void EngineTexture::uploadCube(const std::array<std::string, 6>& cubeArray)
{
    ...
    for (int i = 0; i < 6; i++) {
        int nC;
        faces[i].pixels = stbi_load(cubeArray[i].c_str(),
                                    &faces[i].w,
                                    &faces[i].h,
                                    &nC,
                                    STBI_rgb_alpha);
    ...
    for (int i = 0; i < 6; i++) {
        stagingBuffer.writeToBuffer((void*)faces[i].pixels,
                                    imageSize,
                                    imageSize * i);
...
void EngineTexture::createVkImage(uint32_t texWidth,
                                  uint32_t texHeight,
                                  uint32_t layers,
                                          ...
                                          uint32_t layerCount)
{
    if (VkCommandBuffer commandBuffer = geDevice.beginSingleTimeCommands())
    {
        ...
        if (!setPipelineStageFlags(barrier,
                                   oldLayout,
                                   newLayout,
                                   srcStage,
                                   dstStage))
...
void EngineTexture::createImageView(VkImageViewType viewType,
                                    uint32_t layers,
                                    VkFormat format)
{
                                  ...
                                  VkFilter magFilter)
{
    ...
    samplerInfo.maxAnisotropy =
        geDevice.properties.limits.maxSamplerAnisotropy;
    ...
    if (vkCreateSampler(geDevice.device(),
                        &samplerInfo,
                        nullptr,
