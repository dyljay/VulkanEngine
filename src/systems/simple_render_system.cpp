...
SimpleRenderSystem::SimpleRenderSystem(
    ...
    : RenderSystem{device, shaders, descriptorSetLayouts,
                   sizeof(SimplePushConstantData), renderPass} {}
...
void SimpleRenderSystem::render(FrameInfo &frameInfo,
  ...
  vkCmdBindDescriptorSets(frameInfo.commandBuffer,
                          VK_PIPELINE_BIND_POINT_GRAPHICS, getPipelineLayout(),
                          2, 1, &descriptorSets.cubeMap, 0, nullptr);

  // sample2D array binding
  vkCmdBindDescriptorSets(frameInfo.commandBuffer,
                          VK_PIPELINE_BIND_POINT_GRAPHICS, getPipelineLayout(),
                          3, 1, &descriptorSets.textureArray, 0, nullptr);
  ...
  for (auto &kv : frameInfo.gameObjects) {
    ...
    for (auto &mesh : obj.model->meshes) {
      ...
      vkCmdPushConstants(frameInfo.commandBuffer, getPipelineLayout(),
