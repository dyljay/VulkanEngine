...
void EngineController::moveInXYZPlane(float dt, GameObject& gameObject)
{
    ...
    if (keyState[SDL_SCANCODE_W]) moveDir += forwardDir;
    if (keyState[SDL_SCANCODE_S]) moveDir -= forwardDir;
    if (keyState[SDL_SCANCODE_SPACE]) moveDir += upDir;
    if (keyState[SDL_SCANCODE_LSHIFT]) moveDir -= upDir;
    if (keyState[SDL_SCANCODE_D]) moveDir += rightDir;
    if (keyState[SDL_SCANCODE_A]) moveDir -= rightDir;
...
void EngineController::handleMouseMovements(SDL_Event& e,
                                            float dt,
