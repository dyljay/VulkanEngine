        ...
        std::visit(
            fastgltf::visitor{
                ...
                [&](fastgltf::sources::BufferView& view) {
                    auto& bufferView =
                        gltf.bufferViews[view.bufferViewIndex];
                    auto& bufferFromBufferView =
                        gltf.buffers[bufferView.bufferIndex];
...
void EngineModel::loadMaterials()
{
    ...
    for (int matIndex = 0; matIndex < numMaterials; matIndex++) {
        ...
        {
            material->properties.baseColor = glm::make_vec4(
                gltfMaterial.pbrData.baseColorFactor.data());
        }

        if (gltfMaterial.pbrData.baseColorTexture.has_value()) {
            material->properties.baseColorMap =
                gltfMaterial.pbrData.baseColorTexture.value().textureIndex;
            material->properties.numFeatures |=
                PBRMaterial::GLSL_HAS_BASE_MAP;
        }
        ...
        if (gltfMaterial.normalTexture.has_value()) {
            ...
            material->properties.numFeatures |=
                PBRMaterial::GLSL_HAS_NORMAL_MAP;
        }

        // metallic/roughness values
        {
            material->properties.roughness =
                gltfMaterial.pbrData.roughnessFactor;
            material->properties.metallic =
                gltfMaterial.pbrData.metallicFactor;
        }
        ...
        if (gltfMaterial.pbrData.metallicRoughnessTexture.has_value()) {
            material->properties.metallicRoughness =
                gltfMaterial.pbrData.metallicRoughnessTexture.value()
                    .textureIndex;
            material->properties.numFeatures |=
                PBRMaterial::GLSL_HAS_METAL_MAP;
        }
